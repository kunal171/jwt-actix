...
fn marshall_user(d: &Document) -> Document {
    let k = doc! {
      ...
      "name":d.get("name").unwrap().as_str().unwrap(),
      "bio":d.get("bio").unwrap().as_str().unwrap(),
...
impl UserService {
    ...
    pub fn update(
        ...
        if updates.name.is_some() {
            updates_doc.insert("name", updates.name.unwrap());
        }

        if updates.bio.is_some() {
            updates_doc.insert("bio", updates.bio.unwrap());
        }
