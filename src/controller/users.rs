use crate::models::users::{AuthenticateUser, AuthorizedUser, UpdateUser, User, UserInfo};
use crate::AppState;
...
pub async fn get_single_user(
    ...
    if &requestor.sub != &info.user_id {
        dbg!("requestor.sub: {}", &requestor.sub);
        dbg!("info.user_id: {}", &info.user_id);
        dbg!("info.user_id: {}", &requestor.role);
        dbg!("User did not pass basic logic");
        return HttpResponse::Unauthorized().finish();
    }
